<?php
    class Blog extends CI_Controller {
        ...
        public function index() {
            ...
            $categories = array();
            for($i = 1; $i <= $count; $i++) {
                $query = $this->db->query("SELECT Posts.ID, Theme.Name, Posts.Title, Posts.Cover, Posts.Date FROM `Posts` INNER JOIN `Categories` ON Categories.ID = Posts.Category INNER JOIN `Theme` ON Theme.ID = Posts.Theme WHERE Theme.ID = $i ORDER BY CONVERT(DATE, date) DESC LIMIT 4");
        ...
        public function category() {
            ...
            $query = $this->db->query("SELECT Posts.ID, Categories.Name, Posts.Title, Posts.Cover, Posts.Date FROM `Posts` INNER JOIN `Categories` ON Categories.ID = Posts.Category WHERE Categories.Name = '".$title."' ORDER BY CONVERT(DATE, date) DESC");
        ...
        public function metamorphoses() {
            $query = $this->db->query("SELECT Posts.ID, Categories.Name, Posts.Title, Posts.Cover, Posts.Date FROM `Posts` INNER JOIN `Categories` ON Categories.ID = Posts.Category WHERE Categories.Name = 'Metamorfozy' ORDER BY CONVERT(DATE, date) DESC");
